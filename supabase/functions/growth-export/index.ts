...
import { exportGrowthData } from "./service.ts";
import type { GrowthExportRequest } from "./types.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const access = await requireAdminAccess(req, client, { cronSecretEnv: "GROWTH_EXPORT_SECRET" });
    if (!access.ok) return json(req, { error: access.error || "Admin access required" }, access.status);

    let body: GrowthExportRequest;
    try {
      body = await readJsonBody<GrowthExportRequest>(req, MAX_BODY_BYTES);
    } catch (error) {
      if (error instanceof BodyTooLargeError) return json(req, { error: "Request body too large" }, 413);
      return json(req, { error: "Invalid JSON body" }, 400);
    }

    const result = await exportGrowthData(body, client);
    return json(req, result.body, result.status || 200);
  } catch {
    return json(req, { error: "Growth export failed" }, 500);
  }
});
