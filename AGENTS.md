...
- 设计效果图统一简洁标注“设计效果图”，英文使用“Design rendering”；材质搭配图使用“材质搭配示意” / “Material palette study”。
- 不自动在首屏、图片角标、卡片、正文、弹窗、图片 alt 或 SEO 描述中追加“人工智能概念图”“非完工案例”“非客户完工实景”“不代表已完工项目”等说明；英文同样不追加对应措辞。
- 效果图应按效果图准确呈现，不得改称“实景”“客户实拍”“已完工项目”，也不得据此编造客户、地址或交付事实。
- 生成来源、授权依据和素材记录保留在内部审计资料；公开文案以服务内容和设计方向为主。
- 具体要求见 `docs/rules/public-case-privacy.md` 的“效果图对外标注”。用户明确要求展示的说明按当次要求执行。

## 设计参考来源与共享布局

- 开始规划、绘制效果图或制作页面预览前，先明确本轮用户指定的目标设计，以及当前网站实际使用的公共布局组件；不能把历史方案和当前设计自行混合成新的基准。
- 历史线框、旧效果图、验收截图、独立 HTML 预览和旧 worktree 只作为历史记录；未经本轮用户明确选用，不得当作当前设计要求或共享布局来源。
- 用户提供的设计交接包以本轮指定的正文设计为准。附件中的 `preview` 外壳、仿制导航、页脚和手机底栏不能覆盖网站当前共享组件，即使附件名称包含 Final 或最新版。
- 展示“结合当前网站”的效果时，必须实际复用当前 `src/App.tsx` 挂载的共享组件。仅正文探索的独立预览须明确标为正文原型，不能声称已复用主站公共区域。
- 用户要求沿用线上最新布局时，先核对运行版本与本地基线；无法核实时说明未核实，不得用旧截图、旧源码或附件外壳代替最新布局。
- “沿用现有设计系统”指复用已核实的组件和样式，不代表历史规划稿仍有效。设计正文变更按本轮授权执行，共享区域继续使用当前实现。
...
- SEO/GEO 审核通过的后台内容发布固定走受保护 Edge Function `content-publish`；当前支持 `service`、`service_area`、受限 `homepage`、`blog`、`material`、`project` 与受限 `site_page` 内容类型，可以使用管理员 Bearer token，或使用 `CONTENT_PUBLISH_SECRET` 对应的 `x-cron-secret` 机器密钥。管理员角色必须是 `super_admin` 或 `content_editor`，机器密钥只允许作为内容发布自动化路径，发布模式必须包含 `ownerApproved: true` 和 `explicitExecution: true`。禁止 Codex 或脚本直接写内容数据库表来绕过后台发布链路。
...
- 生产入口统一为 `.github/workflows/cloudflare-pages-deploy.yml`；`npm run deploy:cloudflare:pages` 只派发该入口，不在本地再次构建或直接调用 Wrangler。
