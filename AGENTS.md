...
- 无 PII 推广归因导出固定走受保护 Edge Function `growth-export`。
- 受控推广 Telegram 变更通知固定走受保护 Edge Function `growth-notify`。
- SEO/GEO 审核通过的后台内容发布固定走受保护 Edge Function `content-publish`；当前支持 `service` 与受限 `homepage` 内容类型，可以使用管理员 Bearer token，或使用 `CONTENT_PUBLISH_SECRET` 对应的 `x-cron-secret` 机器密钥。管理员角色必须是 `super_admin` 或 `content_editor`，机器密钥只允许作为内容发布自动化路径，发布模式必须包含 `ownerApproved: true` 和 `explicitExecution: true`。禁止 Codex 或脚本直接写内容数据库表来绕过后台发布链路。
