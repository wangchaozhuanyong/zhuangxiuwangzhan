...
export default function LocationPage() {
  ...
  return (
    <main className="fc-route-page">
      ...
      <SchemeARouteHero kind="detail" image={pageHeroImages.locations.desktop} imageSourceWidth={pageHeroImages.locations.desktopWidth} tabletImage={pageHeroImages.locations.tablet} tabletImageSourceWidth={pageHeroImages.locations.tabletWidth} mobileImage={pageHeroImages.locations.mobile} mobileImageSourceWidth={pageHeroImages.locations.mobileWidth} imagePosition={pageHeroImages.locations.imagePosition} imageAlt={language === "zh" ? `${location.name} 装修服务范围规划示意` : `${location.name} renovation service area planning concept`} label={[copy.breadcrumbLocations, mediaLabels[language].renderingConcept].join(" · ")} title={copy.heroTitle(location.name)} description={display(location.description)} />
      <SchemeAFacts items={[
