...
const getFallbackProjects = async () => (await import("@/data/projects")).projectsData;
...
const getFallbackLocations = async () => (await import("@/data/locations")).locationsData;
...
export const getPublishedProjects = async (language: "en" | "zh") => {
  if (!isSupabaseConfigured) return getFallbackProjects();
  ...
  if (error || !data?.length) return getFallbackProjects();

  return data.map((item: any) => ({
    id: item.id,
    slug: item.slug,
    title: item[`title_${language}`] || item.title_en || item.title_zh,
    type: item.project_type || "Renovation",
    location: item.location || "",
    description:
      item[`excerpt_${language}`] ||
      item.excerpt_en ||
      item.excerpt_zh ||
      stripHtml(item[`content_${language}`] || item.content_en || item.content_zh || ""),
    clientNeed: item[`client_need_${language}`] || item.client_need_en || item.client_need_zh || "",
    materialsUsed: item.materials || [],
    scope: item.scope || [],
    highlights: item[`highlights_${language}`] || item.highlights_en || item.highlights_zh || [],
...
export const getPublishedProjectBySlug = async (slug: string, language: "en" | "zh") => {
  const fallbackProjects = async () => (await getFallbackProjects()).find((project) => project.slug === slug) || null;
  ...
  return {
    ...
    type: data.project_type || "Renovation",
    location: data.location || "",
    description: data[`content_${language}`] || data.content_en || data.content_zh || "",
    clientNeed: data[`client_need_${language}`] || data.client_need_en || data.client_need_zh || "",
    materialsUsed: data.materials || [],
    scope: data.scope || [],
    highlights: data[`highlights_${language}`] || data.highlights_en || data.highlights_zh || [],
...
export const getPublishedServiceAreaBySlug = async (slug: string, language: "en" | "zh" = "en") => {
  const fallback = async () => {
    const locationsData = await getFallbackLocations();
  ...
  return {
    name: data.area_name || data[`title_${language}`] || data.title_en || data.title_zh,
    slug: data.slug,
    metaTitle: data[`seo_title_${language}`] || data.seo_title_en || data.seo_title_zh || data.area_name,
    description: data[`seo_description_${language}`] || data.seo_description_en || data.seo_description_zh || data[`excerpt_${language}`] || "",
    intro: data[`content_${language}`] || data.content_en || data.content_zh || "",
    propertyTypes: data.property_types || [],
    commonNeeds: data.common_needs || [],
    constructionNotes: data[`construction_notes_${language}`] || data.construction_notes_en || data.construction_notes_zh || "",
