import { invokeGeocodeAddressFunction } from "@/backend/modules/system/repository/geocodeRepository";
...
export const geocodeAdminAddress = async (address: string): Promise<GeocodeResult> => {
  ...
  let data: unknown;
  try {
    data = await invokeGeocodeAddressFunction(cleanAddress);
  } catch {
