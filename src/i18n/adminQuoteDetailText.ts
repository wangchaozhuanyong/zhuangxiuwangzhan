export const adminQuoteDetailText = {
  defaultFieldLabel: { en: "Content", zh: "内容" },
  readOnlyQuote: { en: "This account has a read-only role and cannot edit quote content.", zh: "当前账号是只读角色，不能修改报价内容。" },
  savingField: { en: "{label} saving...", zh: "{label}保存中..." },
  saveFieldFailed: { en: "{label} save failed: {reason}", zh: "{label}保存失败：{reason}" },
  savedField: { en: "{label} saved.", zh: "{label}已保存。" },
  readOnlyFollowup: { en: "This account has a read-only role and cannot add follow-up records.", zh: "当前账号是只读角色，不能新增跟进记录。" },
  followupContentRequired: { en: "Fill in follow-up content first.", zh: "请先填写跟进内容。" },
  followupSavedSyncFailed: {
    en: "Follow-up saved, but next follow-up time sync failed: {reason}",
    zh: "跟进已保存，但下次跟进时间同步失败：{reason}",
  },
  followupSaved: { en: "Follow-up record saved.", zh: "跟进记录已保存。" },
  pageTitle: { en: "Quote details", zh: "报价详情" },
  pageDescription: { en: "View quote requests, fill in quoted amount, update status, and add follow-up records.", zh: "查看报价请求、填写报价金额、更新状态和跟进记录。" },
  pageHelpText: {
    en: "This is the handling page for one quote request. You can fill in quote amount, schedule a site visit, and add notes here.",
    zh: "这里是单条报价请求的处理页面。你可以在这里填报价、安排上门、补说明。",
  },
  quoteFallback: { en: "Quote request", zh: "报价请求" },
  call: { en: "Call", zh: "拨打电话" },
  detailSection: { en: "Quote details", zh: "报价详情" },
  location: { en: "Location:", zh: "地点：" },
  propertySize: { en: "Area / unit type:", zh: "面积/户型：" },
  estimatedBudget: { en: "Estimated budget:", zh: "预算预估：" },
  quotedAmount: { en: "Quoted amount", zh: "报价金额" },
  projectDetails: { en: "Project details:", zh: "项目详情：" },
  status: { en: "Status", zh: "状态" },
  leadQuality: { en: "Lead quality", zh: "询盘质量" },
  qualifiedAt: { en: "Qualified at:", zh: "确认有效时间：" },
  attribution: { en: "Attribution", zh: "推广归因" },
  firstTouch: { en: "First touch:", zh: "首次来源：" },
  lastTouch: { en: "Last touch:", zh: "最后来源：" },
  campaign: { en: "Campaign:", zh: "广告系列：" },
  landingPage: { en: "Landing page:", zh: "首次落地页：" },
  validUntil: { en: "Valid until", zh: "有效期至" },
  validUntilField: { en: "Valid until", zh: "有效期" },
  nextFollowUp: { en: "Next follow-up", zh: "下次跟进" },
  notes: { en: "Notes", zh: "备注" },
  addFollowup: { en: "Add follow-up", zh: "新增跟进" },
  followupPlaceholder: { en: "Follow-up record...", zh: "跟进记录..." },
  saving: { en: "Saving...", zh: "保存中..." },
  saveFollowup: { en: "Save follow-up", zh: "保存跟进" },
  timeline: { en: "Timeline", zh: "时间线" },
  nextFollowUpAt: { en: "Next follow-up: {time}", zh: "下次跟进：{time}" },
} as const;

export const adminQuoteFollowupTypeLabels = {
  note: { en: "Note", zh: "备注" },
  call: { en: "Call", zh: "电话" },
  whatsapp: { en: "WhatsApp", zh: "WhatsApp" },
  site_visit: { en: "Site visit / measurement", zh: "上门/测量" },
  quotation: { en: "Quotation", zh: "报价" },
  closed: { en: "Closed", zh: "结案" },
} as const;
